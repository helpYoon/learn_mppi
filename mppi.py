...
class MPPI(nn.Module):
    ...
    def __init__(
        ...
        :param dynamics_model: Trained transition model
        ...
        if self._dynamics_model is None:
            ...
            self._dynamics = self.transition_dynamics
    ...
    def transition_dynamics(self, state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        th = state[:, 0].view(-1, 1)
        thdot = state[:, 1].view(-1, 1)
        input_state = torch.cat((torch.cos(th), torch.sin(th), thdot), dim=-1).double()
        u = action[:, 0].view(-1, 1)
        u = torch.clamp(u, -2, 2).double()

        state = self._dynamics_model(input_state, u)
        
        th = torch.atan2(state[:, 1], state[:, 0])
        state = torch.stack((th, state[:, 2]), dim=1).double()
    ...
    #     th = state[:, 0].view(-1, 1)
    #     thdot = state[:, 1].view(-1, 1)
    #     input_state = torch.cat((torch.cos(th), torch.sin(th), thdot), dim=-1)
    #     u = action[:, 0].view(-1, 1)
    #     u = torch.clamp(u, -2, 2).double()
